...
import SedanImage from '../assets/SedanImage.jpg';
import { ProductContext } from '../context/ProductContext';
import axios from 'axios';

const SedanWheel = ({ isLoggedIn }) => {
  ...
  useEffect(() => {
    // Filter products by "Sedan" category
    if (products) {
      const sedanwheelProducts = products.filter((product) => product.category === 'Sedan');
      setFilteredProducts(sedanwheelProducts);
    }
  }, [products]);
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (!searchQuery.trim()) {
      // If search is empty, show all Sedan products
      setFilteredProducts(products.filter((product) => product.category === 'Sedan'));
    } else {
        ...
        <div className="relative w-full h-[700px] bg-no-repeat bg-center bg-cover">
          <div className="absolute inset-0">
            <div
              ...
              style={{ backgroundImage: `url(${SedanImage})`, opacity: 0.3, zIndex: -1 }}
            />
          </div>
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
            <h1 className="text-4xl font-bold">Sedan Wheels</h1>
            <p className="mt-4 text-lg text-gray-400">
              Discover the best Sedan Wheels for immersive sound experiences.
            </p>
          </div>
        </div>
      )}

...
import SUVImage from '../assets/SUVImage.jpg';
import { ProductContext } from '../context/ProductContext';
import axios from 'axios';

const SUVWheel = ({ isLoggedIn }) => {
  ...
  useEffect(() => {
    // Filter products by "SUV" category
    if (products) {
      const suvwheelProducts = products.filter((product) => product.category === 'SUV');
      setFilteredProducts(suvwheelProducts);
    }
  }, [products]);
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (!searchQuery.trim()) {
      // If search is empty, show all SUV products
      setFilteredProducts(products.filter((product) => product.category === 'SUV'));
    } else {
        ...
        <div className="relative w-full h-[700px] bg-no-repeat bg-center bg-cover">
          <div className="absolute inset-0">
            <div
              ...
              style={{ backgroundImage: `url(${SUVImage})`, opacity: 0.3, zIndex: -1 }}
            />
          </div>
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
            <h1 className="text-4xl font-bold">SUV Wheels</h1>
            <p className="mt-4 text-lg text-gray-400">
              Discover the best SUV Wheels for immersive sound experiences.
            </p>
          </div>
        </div>
      )}

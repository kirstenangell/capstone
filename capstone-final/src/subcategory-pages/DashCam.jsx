...
import DashCamImage from '../assets/DashCamImage.jpg'; // Replace with the path to your image
...
const DashCam = () => {
  ...
  // Filter products to only show those with the category 'Dash Cam'
  useEffect(() => {
    const dashCamProducts = products.filter((product) => product.category === 'Dash Cam');
    setFilteredProducts(dashCamProducts);
  }, [products]);

  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim() === '') {
      setFilteredProducts(products.filter((product) => product.category === 'Dash Cam'));
    } else {
    ...
    <div className="relative min-h-screen bg-black text-white">
      ...
      <div className="relative w-full h-[700px] bg-no-repeat bg-center bg-cover">
        <div className="absolute inset-0">
          <div
            className="w-full h-full bg-no-repeat bg-center bg-cover mt-36"
            style={{ backgroundImage: `url(${DashCamImage})`, opacity: 0.2, zIndex: -1 }}
          />
        </div>
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
          <h1 className="text-4xl font-bold">Dash Cam</h1>
          <p className="mt-4 text-lg text-gray-400">
            Explore our premium dash cameras for your vehicle.
          </p>
        </div>
      </div>

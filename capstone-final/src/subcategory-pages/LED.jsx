...
import LEDImage from '../assets/LEDImage.jpg'; // Replace with the path to your image
...
const LED = () => {
  ...
  // Filter products to only show those with the category 'LED'
  useEffect(() => {
    const ledProducts = products.filter((product) => product.category === 'LED');
    setFilteredProducts(ledProducts);
  }, [products]);

  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim() === '') {
      setFilteredProducts(products.filter((product) => product.category === 'LED'));
    } else {
      ...
      <div className="relative w-full h-[700px] bg-no-repeat bg-center bg-cover">
        <div className="absolute inset-0">
          <div
            ...
            style={{ backgroundImage: `url(${LEDImage})`, opacity: 0.2, zIndex: -1 }}
          />
        </div>
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
          <h1 className="text-4xl font-bold">LED</h1>
          <p className="mt-4 text-lg text-gray-400">
            Discover the best LED products for your needs.
          </p>
        </div>
      </div>

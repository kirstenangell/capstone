...
import ReverseCameraImage from '../assets/ReverseCameraImage.jpg';
import { ProductContext } from '../context/ProductContext';
import axios from 'axios';

const ReverseCamera = ({ isLoggedIn }) => {
  ...
  useEffect(() => {
    // Filter products by "Reverse Camera" category
    if (products) {
      const reversecameraProducts = products.filter((product) => product.category === 'Reverse Camera');
      setFilteredProducts(reversecameraProducts);
    }
  }, [products]);
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (!searchQuery.trim()) {
      // If search is empty, show all Reverse Camera products
      setFilteredProducts(products.filter((product) => product.category === 'Reverse Camera'));
    } else {
        ...
        <div className="relative w-full h-[700px] bg-no-repeat bg-center bg-cover">
          <div className="absolute inset-0">
            <div
              ...
              style={{ backgroundImage: `url(${ReverseCameraImage})`, opacity: 0.3, zIndex: -1 }}
            />
          </div>
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
            <h1 className="text-4xl font-bold">Reverse Camera</h1>
            <p className="mt-4 text-lg text-gray-400">
              Discover the best Reverse Camera for immersive sound experiences.
            </p>
          </div>
        </div>
      )}

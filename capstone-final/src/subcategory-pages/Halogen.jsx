...
import HalogenImage from '../assets/HalogenImage.jpg'; // Replace with the path to your image
...
const Halogen = () => {
  ...
  // Filter products to only show those with the category 'Halogen'
  useEffect(() => {
    const halogenProducts = products.filter((product) => product.category === 'Halogen');
    setFilteredProducts(halogenProducts);
  }, [products]);

  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim() === '') {
      setFilteredProducts(products.filter((product) => product.category === 'Halogen'));
    } else {
      ...
      <div className="relative w-full h-[700px] bg-no-repeat bg-center bg-cover">
        <div className="absolute inset-0">
          <div
            ...
            style={{ backgroundImage: `url(${HalogenImage})`, opacity: 0.2, zIndex: -1 }}
          />
        </div>
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
          <h1 className="text-4xl font-bold">Halogen</h1>
          <p className="mt-4 text-lg text-gray-400">
            Explore our premium halogen lights for your vehicles.
          </p>
        </div>
      </div>

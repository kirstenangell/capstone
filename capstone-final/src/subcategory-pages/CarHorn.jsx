...
import CarHornImage from '../assets/CarHornImage.jpg';
import { ProductContext } from '../context/ProductContext';
import axios from 'axios';

const CarHorn = ({ isLoggedIn }) => {
  ...
  useEffect(() => {
    // Filter products by "Car Horn" category
    if (products) {
      const carhornProducts = products.filter((product) => product.category === 'Car Horn');
      setFilteredProducts(carhornProducts);
    }
  }, [products]);
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (!searchQuery.trim()) {
      // If search is empty, show all car horn products
      setFilteredProducts(products.filter((product) => product.category === 'Car Horn'));
    } else {
        ...
        <div className="relative w-full h-[700px] bg-no-repeat bg-center bg-cover">
          <div className="absolute inset-0">
            <div
              ...
              style={{ backgroundImage: `url(${CarHornImage})`, opacity: 0.3, zIndex: -1 }}
            />
          </div>
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
            <h1 className="text-4xl font-bold">Car Horn</h1>
            <p className="mt-4 text-lg text-gray-400">
              Discover the best Car Horn for immersive sound experiences.
            </p>
          </div>
        </div>
      )}

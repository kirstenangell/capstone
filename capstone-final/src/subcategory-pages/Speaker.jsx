...
import SpeakerImage from '../assets/SpeakerImage.jpg'; // Replace with the path to your image
...
const Speaker = () => {
  ...
  // Filter products to only show those with the category 'Speaker'
  useEffect(() => {
    const speakerProducts = products.filter((product) => product.category === 'Speakers');
    setFilteredProducts(speakerProducts);
  }, [products]);

  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim() === '') {
      setFilteredProducts(products.filter((product) => product.category === 'Speakers'));
    } else {
      ...
      <div className="relative w-full h-[700px] bg-no-repeat bg-center bg-cover">
        <div className="absolute inset-0">
          <div
            ...
            style={{ backgroundImage: `url(${SpeakerImage})`, opacity: 0.2, zIndex: -1 }}
          />
        </div>
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
          <h1 className="text-4xl font-bold">Speaker</h1>
          <p className="mt-4 text-lg text-gray-400">
            Discover the best speakers for your sound system.
          </p>
        </div>
      </div>
